...
export const actions = {
  ...
  async getCommentList ({ commit }, requestData) {
    try {
      let { data, headers } = await this.$axios.$get(`/wp-json/wp/v2/comments`, {
        params: requestData,
        data: { progress: false }
      })
  ...
  async updateComment ({ commit }, requestData) {
    try {
      ...
      let { data } = await this.$axios.$post(`/wp-json/wp/v2/comments`, requestData, {
        headers: {
          progress: false,
        }
      })
  ...
  async getExpression ({ commit, rootState }) {
    try {
      let { data } = await this.$axios.$get(`${rootState.info.wp + rootState.info.templeteUrl}/expression.php`, {
        data: { progress: false }
      })
  ...
  async updateCommentOpinion ({ commit }, requestData) {
    try {
      let { data } = await this.$axios.$post(`/wp-json/xm-blog/v1/update-comment-meta`, requestData, {
        headers: {
          progress: false
        }
      })
