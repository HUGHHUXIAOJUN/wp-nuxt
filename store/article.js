...
export const actions = {
  ...
  async getArticleList ({ rootState, commit }, requestData) {
    try {
      let { data, headers } = await this.$axios.$get(`/wp-json/wp/v2/posts`, {
        params: requestData,
        data: { progress: false }
      });
      data.map(item => {
  ...
  async getArticleDetail ({ commit, rootState }, id) {
    try {
      ...
      let { data } = await this.$axios.$get(`/wp-json/wp/v2/posts/${id}`, {
        data: { progress: false }
      })
  ...
  async updateArticleViewCount ({ commit }, requestData) {
    try {
      let { data } = await this.$axios.$post(`/wp-json/xm-blog/v1/view-count`, requestData, {
        headers: {
          progress: false
        }
      })
  ...
  async updateOpinion ({ commit }, requestData) {
    try {
      let { data } = await this.$axios.$post(`/wp-json/xm-blog/v1/like`, requestData, {
        headers: {
          progress: false
        }
      })
